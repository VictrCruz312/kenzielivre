...
export const updateUser = ( userUpdate:IUserVendedor, idUser:number ):Promise<IReturnData> => {
    
    return  api.patch(`/users/${ idUser }`, userUpdate, config() )
                .then( ({ data }) => data )
}
...
export const deleteUser = ( idUser:number ):Promise<object> => {
    ...
    return  api.delete(`/users/${ idUser }`, config() )
                .then( ({ data }) => data )
}
